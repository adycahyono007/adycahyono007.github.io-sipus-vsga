...
function getTampilBuku($pg, $req)
{
	...
	if(mysqli_num_rows($q_tampil_buku)>0)
	{
		while($r_tampil_buku=mysqli_fetch_array($q_tampil_buku))
		{
			...
			echo "<td>" . strtoupper($r_tampil_buku['statusbuku']) . "</td>";
			echo "<td>";
			
			echo "<div class='btn-group btn-group-sm'>";
			echo "<a href=pages/cetak_kartu_buku.php?id=$r_tampil_buku[idbuku]  target=_blank class='btn btn-outline-info' role='button'>Cetak Data Buku</a>";
	...
	if(mysqli_num_rows($q_tampil_buku)>0)
	{
		while($r_tampil_buku=mysqli_fetch_array($q_tampil_buku))
		{
			...
			*/
			...
			echo "<p>";
			echo "Kategori: " .strtoupper($r_tampil_buku['kategori']);
			echo ", Pengarang: " . strtoupper($r_tampil_buku['pengarang']) ;
			echo ", Penerbit: " . strtoupper($r_tampil_buku['penerbit']) ;
			echo ", Status Buku: " . strtoupper($r_tampil_buku['statusbuku']) ;
			echo "</p>";
...
{	
	...
	if(mysqli_num_rows($q_tampil_buku)>0)
	{
		while($r_tampil_buku=mysqli_fetch_array($q_tampil_buku))
		{
			...
			echo "<tr>";
			echo "<td>" . $nomor . "</td>";
			echo "<td>" . $r_tampil_buku['idbuku'] . "</td>";
			echo "<td>" . strtoupper($r_tampil_buku['judulbuku']) . "</td>";
			echo "<td>" . strtoupper($r_tampil_buku['kategori']) . "</td>";
			echo "<td>" . strtoupper($r_tampil_buku['pengarang']) . "</td>";
			echo "<td>" . strtoupper($r_tampil_buku['penerbit']) . "</td>";
...
/* modif dengan menyiman nama file asli dari foto */
function processAddBook ($arr=array(), $post)
{
	if(isset($post)){
		...
		{
			$sql = " INSERT INTO tbbuku VALUES ('$idbuku','$judulbuku','$kategori','$pengarang','$penerbit', '$statusbuku')";
			$query = mysqli_query(getConnection(), $sql);
		}
...
function processUpdateBook ($arr=array(), $post)
{
	if (isset($post)){
		...
		// periksa apakah data id_buku, judulbuku tidak kosong
		if (!empty($id_buku) && !empty($judul_buku))
		{
			// lakukan proses perubahan data ke database
			mysqli_query(getConnection(),
			"UPDATE tbbuku
			SET judulbuku='$judul_buku',kategori='$kategori',pengarang='$pengarang',penerbit='$penerbit',statusbuku='$status_buku'
...
function getBookForUpdate ($get)
{
	...
	$arr = array(	"id_buku"=>$r_tampil_buku['idbuku'], 
					"judul_buku"=>$r_tampil_buku['judulbuku'], 
					"kategori"=>$r_tampil_buku['kategori'], 
					"pengarang"=>$r_tampil_buku['pengarang'], 
					"penerbit"=>$r_tampil_buku['penerbit'], 
					"statusbuku"=>$r_tampil_buku['statusbuku'], 
					"page"=>$hal);

	return $arr;
}

function getPrintKartuBook ($post)
{
	$id_buku = $post;

	$q_tampil_buku=mysqli_query(getConnection(), "SELECT * FROM tbbuku WHERE idbuku='$id_buku'");
	$r_tampil_buku=mysqli_fetch_array($q_tampil_buku);
